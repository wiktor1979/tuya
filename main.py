...
def daily_report_loop():
    ...
    # Przelicz godzinę lokalną na godzinę serwera
    server_hour = (DAILY_REPORT_HOUR + SERVER_TIMEZONE_OFFSET) % 24
    print(f"Uruchomiono watek raportu dziennego (lokalnie: {DAILY_REPORT_HOUR}:00, serwer: {server_hour}:00, offset: {SERVER_TIMEZONE_OFFSET}h)", flush=True)
    last_report_date = None

    while True:
        now = datetime.now()

        # Porównuj z godziną serwera
        if now.hour == server_hour and now.date() != last_report_date:
            print(f"[{time.strftime('%H:%M:%S')}] Generowanie raportu dziennego...", flush=True)
